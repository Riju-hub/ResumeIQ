...
def generate_improvements(
    ...
    if score_results.get('formatting_score', 0) < 14:
        improvements.append({
            "issue_title": "Improve Resume Formatting",
            "severity_level": "Medium",
            "ats_impact": "High",
            "explanation": "Add standard bullet points and clearly structured headings.",
            "how_to_fix": "Use clean bullet lists and ensure sections like Experience and Education are clearly demarcated."
        })

    if score_results.get('keywords_score', 0) < 16:
        improvements.append({
            "issue_title": "Increase Keyword Density",
            "severity_level": "High",
            "ats_impact": "Critical",
            "explanation": "Resume lacks targeted technical keywords matching industry roles.",
            "how_to_fix": "Incorporate key technical skills and domain terminology into your experience descriptions."
        })

    if score_results.get('content_score', 0) < 16:
        improvements.append({
            "issue_title": "Quantify Achievements",
            "severity_level": "Medium",
            "ats_impact": "Moderate",
            "explanation": "Action verbs and numeric metrics (%, $, scale) are sparse.",
            "how_to_fix": "Start bullets with active verbs and quantify impact (e.g., 'Reduced build times by 35%')."
        })

    unvalidated_count = len(skill_validation_results.get('unvalidated_skills', []))
    if unvalidated_count > 0 and score_results.get('skill_validation_score', 0) < 12:
        improvements.append({
            "issue_title": "Unsubstantiated Skills",
            "severity_level": "Low",
            "ats_impact": "Moderate",
            "explanation": f"{unvalidated_count} skill(s) listed in the skills block lack matching evidence in project or work history.",
            "how_to_fix": "Mention these skills in the context of your project bullet points."
        })
