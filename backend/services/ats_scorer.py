...
#     # method 03: ZIP code regex pattern
...
# # High-performance batch skill validation
...
#         if matched:
#             validated_skills.append({'skill': skill, 'projects': matched, 'similarity': 1.0})
#             skill_project_mapping[skill] = matched
#         else:
#             skills_to_embed.append(skill)

#     # Batch embedding inference for remaining skills
#     if skills_to_embed and targets:
#         try:
#             skill_vecs = embedder.encode(skills_to_embed, batch_size=32, convert_to_numpy=True, normalize_embeddings=True)
#             target_vecs = embedder.encode([t[1][:2000] for t in targets], batch_size=16, convert_to_numpy=True, normalize_embeddings=True)
...
#                 if matched_names:
#                     validated_skills.append({'skill': skill, 'projects': matched_names, 'similarity': max_sim})
#                     skill_project_mapping[skill] = matched_names
#                 else:
#                     unvalidated_skills.append(skill)
#                     skill_project_mapping[skill] = []
#         except Exception as e:
#             log_warning(f"Batch embedding validation failed: {e}", context='ats_scorer')
...
# High-performance, low-RAM batch skill validation
def validate_skills_with_projects(
    skills: List[str],
    projects: List[Dict],
    experience_entries: List[Dict],
    ...
    if skills_to_embed and targets:
        try:
            with torch.inference_mode():
                # Truncate text targets to 500 chars to avoid memory expansion
