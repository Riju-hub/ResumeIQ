...
def extract_text_from_pdf(file_data: bytes) -> str:
    try:
        ...
        raise FileParsingError(
            'Failed to extract text from PDF. Ensure the file contains selectable text.'
        ) from e
...
def extract_text_from_docx(file_data: bytes) -> str:
    try:
        ...
        if not text.strip():
            raise FileParsingError(
                'No text could be extracted from DOCX. The document may be empty.'
            )
        ...
        raise FileParsingError(
            'Failed to extract text from DOCX. Try re-saving or converting to PDF.'
        ) from e


def extract_text_from_doc(file_data: bytes) -> str:
    raise FileParsingError(
        'Legacy .doc format is not supported. Please convert your document to .docx or .pdf.'
    )
...
def extract_text(file_data: bytes, file_type: str) -> str:
    if file_type == 'pdf':
        ...
        raise FileValidationError(
            f'Invalid file type: {file_type}. Supported types are: pdf, docx, doc.'
        )
